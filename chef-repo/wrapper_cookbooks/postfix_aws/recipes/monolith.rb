...
machine "mail.#{app['domain']}" do
  ...
  machine_options({
    bootstrap_options: {
      ...
      security_group_ids: ['webserver', 'mail'],
...
elastic_ip = aws_eip_address "mail.#{app['domain']}" do
  machine "mail.#{app['domain']}"
end
